...
void setup() {
  ...
  smooth();
  myPort = new Serial(this, "COM3", 115200);  // CHANGE TO YOUR PORT
  myPort.bufferUntil('\n');  // Wait for newline instead of '.'
}
...
void drawText() {
  ...
  int[] rots = {-60, -30, 0, -30, -60};

  for (int i = 0; i < angles.length; i++) {
    resetMatrix();
    float tx = (width - width * 0.5) + width/2 * cos(radians(degs[i]));
    float ty = (height - height * 0.09) - width/2 * sin(radians(degs[i]));
    translate(tx, ty);
    rotate(radians(rots[i]));
    text(angles[i], 0, 0);
  }
